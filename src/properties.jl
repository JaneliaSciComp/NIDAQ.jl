...
function getproperties(t::TaskHandle; warning=false)
    getproperties_guts((t,), "Task", warning)
end
...
function getproperties(t::TaskHandle, channel::ASCIIString; warning=false)
    kind = channel_types[ find(channel_type(t, channel)[1] .==
            map((x)->getfield(NIDAQ,symbol(x)), channel_types))[1]][end-1:end]

    getproperties_guts((t, convert(Ptr{Uint8},channel)), kind, warning)
end
